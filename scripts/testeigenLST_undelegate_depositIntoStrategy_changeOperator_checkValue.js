...
module.exports = async function (callback) {
    try {
        ...
        let userUnderlying = BigNumber(await eigenStrategy.userUnderlyingView(eigenLSTRestakingAddr));
        console.log("userUnderlying: ", userUnderlying.div(1e18).toString(10));
        ...
        let userUnderlying4 = BigNumber(await eigenStrategy.userUnderlyingView(eigenLSTRestakingAddr));
        console.log("userUnderlying4: ", userUnderlying4.div(1e18).toString(10));

        let shares4 = BigNumber(await eigenStrategy.shares(eigenLSTRestakingAddr));
        console.log("shares4: ", shares4.div(1e18).toString(10));
        chai.assert.isTrue(Math.abs(userUnderlying4.div(2).minus(userUnderlying)) < 100, 'Absolute difference should be less than 100');
        chai.assert.isTrue(Math.abs(shares4.div(2).minus(shares)) < 100, 'Absolute difference should be less than 100');
        let amt1 = BigNumber(await eigenLSTRestaking.getAllValue.call({ from: controllerAddr }));
        chai.assert.isTrue(Math.abs(eth_deposit_amount.minus(amt1)) < 100, 'Absolute difference should be less than 100');
