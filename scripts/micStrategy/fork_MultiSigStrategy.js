...
module.exports = async function (callback) {
    try {
        ...
        //     value: repayToAssetVault.toString(10)
        // });
        ...
        try {
            ...
            if (multiSigStrategyValue.gt(0)) {
                ...
                if (repayInterestAmount.gt(0)) {
                    ...
                    const currentTotalValue1 = new BigNumber(currentTotalValueRaw1);
                    if (!currentTotalValue1.eq(currentTotalValue + repayInterestAmount)) {
        ...
        const expectPrice = (multiSigStrategyValue + eigenStrategyValue2 + assetsVaultBalance2 - withdrawableAmountInPast2) / (totalSupply - withdrawingSharesInPast2);
        ...
        if (!multiSigStrategyValue.eq((currentTotalValue1 + eigenStrategyValue1 - shouldClaim) * 0.6)) {
            throw new Error(`策略分配失败`);
        }
        if (!eigenStrategyValue2.eq((currentTotalValue1 + eigenStrategyValue1 - shouldClaim) * 0.4)) {
            throw new Error(`策略分配失败`);
        }
