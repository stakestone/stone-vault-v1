...
const el_ratio = new BigNumber(0.98);
const native_ratio = new BigNumber(0.01);
const sym_ratio = new BigNumber(0.01);
...
const impersonateAccount = async (account) => {
    ...
    await web3.currentProvider.send({
        ...
        params: [account, "0x56BC75E2D63100000"], // Fund with some ETH
        id: Date.now()
    });
    await web3.currentProvider.send({
        jsonrpc: "2.0",
...
module.exports = async function (callback) {
    ...
    try {
        ...
        let withdrawShareAmount = toWei(1000);
        await stone.approve(stoneVault.address, BigNumber(100000).times(1e18), {
            from: testUser
        });
        await stoneVault.requestWithdraw(withdrawShareAmount.toFixed(0), { from: testUser });
        ...
        let topic = "0x26b2aae26516e8719ef50ea2f6831a2efbd4e37dccdf0f6936b27bc08e793e30"; // SlashingWithdrawalQueued event topic
        let log;
        ...
        if (!log) {
            throw new Error("SlashingWithdrawalQueued event log not found.");
        }
        ...
        //因为Lido取款没有真实拿回，为了满足结算，直接充值给originalEigenLSRAddr，所以这样SC值会增加，从而share price也会增加
        diff = fromWei(valuesAfterSwapToEther.sc_totalValue).minus(fromWei(CompleteQueuedWithdrawal.sc_totalValue)).minus(fromWei(totalToFund)).abs();
        assert(
            diff.lte(tolerance),
            `valuesAfterSwapToEther.sc_totalValue差值超出允许范围！实际差值: ${diff.toString(10)}，允许最大值: ${tolerance.toString(10)}`
        );
        ...
        console.log("--- Step 7: Execute rollToNextRound ---");
        ...
        diff = scValueAfterRebalance.times(el_ratio)
            .minus(valuesAfterRebalance.oEGLS_getAllValue)
            .abs();
        assert(
            diff.lte(tolerance),
            `valuesAfterRebalance.oEGLS_getAllValue差值超出允许范围！实际差值: ${diff.toString(10)}，允许最大值: ${tolerance.toString(10)}`
        );
        let alpha_diff = valuesAfterSwapToEther.oEGLS_ETHBalance
            .minus(valuesAfterRebalance.oEGLS_ETHBalance);
        let delta_diff = valuesAfterSwapToEther.oEGLS_getAllValue
            .minus(valuesAfterRebalance.oEGLS_getAllValue);
        assert(
            alpha_diff.minus(delta_diff).lte(tolerance),
            `valuesAfterRebalance.oEGLS_ETHBalance差值超出允许范围！实际前后差值: ${alpha_diff.toString(10)}}`
        );
