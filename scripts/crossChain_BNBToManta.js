...
module.exports = async function (callback) {
    try {
        ...
        const stone_bnb = await Stone.at("0x80137510979822322193FC997d400D5A6C747bf7");

        const amount = BigNumber(2e13).toString(10);

        const packedData = ethers.utils.solidityPack(["bytes"], ["0x"]);

        let feePart = await stone_bnb.estimateSendFee(
            manta_linkId,
            to_address,
            amount,
            false,
        ...
        let result = await stone_bnb.sendFrom(
            from_address,
            manta_linkId,
            to_address,
            amount,
