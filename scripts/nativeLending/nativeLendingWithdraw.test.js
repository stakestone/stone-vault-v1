...
module.exports = async function (callback) {
    try {
        ...
        let tx = await nativeLendingETHStrategy.withdrawFromNativeByAmount(eth_deposit_amount, {
            from: owner
        });
        // Capture the event from the transaction receipt
        const WithdrawByAmountEvent = tx.logs.find(log => log.event === 'WithdrawByAmount');
        if (WithdrawByAmountEvent) {
            const { amount, withdrawAmount } = WithdrawByAmountEvent.args;
            console.log("WithdrawByAmountEvent is: ", WithdrawByAmountEvent);

            console.log("request amount: ", amount.toString());
            console.log("withdraw amount: ", withdrawAmount.toString());
            assert.strictEqual(amount.toString(10), withdrawAmount.toString(10));
            assert.strictEqual(eth_deposit_amount.toString(10), withdrawAmount.toString(10));

        } else {
            console.log("No WithdrawByAmountEvent found");
        }
