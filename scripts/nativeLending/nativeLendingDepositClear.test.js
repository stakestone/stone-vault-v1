...
module.exports = async function (callback) {
    try {
        ...
        const tx2 = await nativeLendingETHStrategy.depositIntoNative(eth_deposit_amount.times(2), {
            from: owner
        });
        ...
        console.log("controllerETHBalance aft clear: ", controllerETHBalance_aft.toString(10));
        chai.assert.isTrue(Math.abs(controllerETHBalance.minus(controllerETHBalance_aft).minus(gas0).minus(gas1).minus(gas3).minus(eth_deposit_amount.times(2))) < 4e14, 'Absolute difference should be less than 100');
